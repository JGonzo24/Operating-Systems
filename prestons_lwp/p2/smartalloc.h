...
inline void *operator new(size_t size, char *file, int line, char pat)throw()
	 { return smartalloc(size, file, line, pat); }
inline void *operator new[](size_t size, char *file, int line, char pat)throw()
	{ return smartalloc(size, file, line, pat); }
...
inline void operator delete[](void *p) throw() 
		{ smartfree(p, __FILE__, __LINE__); }
inline void *operator new(size_t size, const std::nothrow_t&t, 
	char *file, int line, char pat) { 
		return smartalloc(size, file, line, pat); }
inline void *operator new[](size_t size, const std::nothrow_t&t, 
	char *file, int line, char pat) 
		{ return smartalloc(size, file, line, pat); }
inline void *operator new(size_t size) throw (std::bad_alloc) 
		{ return smartalloc(size, __FILE__, __LINE__, 0x54); }

inline void *operator new(size_t size, const std::nothrow_t&t) throw() 
		{ return smartalloc(size, __FILE__, __LINE__, 0x54); }
inline void *operator new[](size_t size, const std::nothrow_t&t) throw()
		 { return smartalloc(size, __FILE__, __LINE__, 0x54); }

inline void *operator new(size_t size, const void *p, 
			const char *file, int line, char pat)
{
	char *data = (char*)p;
...
class STLsmartalloc
{
	public:
		...
		{
			void* p = smartalloc(n * sizeof(T), __FILE__, 
							__LINE__, 0x55);
			if (!p)
				throw std::bad_alloc();
...
namespace SMA {
   ...
   template <class Key, class Data, class Compare = std::less<Key> >
      class map : public std::map<Key, Data, Compare, STLsmartalloc< 
